...
// 주문 받기 상황극 화면: STT 음성 인식으로 답변 후 서버에 학습 세션 결과 저장
class OrderScreen extends StatefulWidget {
  final int categoryId;
  const OrderScreen({super.key, required this.categoryId});
...
class _OrderScreenState extends State<OrderScreen> {
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              children: [
                ...
                _situationBox('# 주문 받기 # 기본 응대'),
                const SizedBox(height: 6),
                _character('assets/images/coffee_girl.jpg'),
                const SizedBox(height: 16),
                Text('" ${current['guide']!} "',
                    style: const TextStyle(fontSize: 17)),
                const SizedBox(height: 4),
                Text(
                  '${currentIndex + 1} / ${scenarios.length} 단계',
                  style: const TextStyle(
  ...
  Future<void> increaseRoleplayCount() async {
    ...
    final response = await http.patch(
      Uri.parse('http://3.37.239.105:8080/api/users/statistics/roleplay'),      headers: {
