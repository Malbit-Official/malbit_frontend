...
// 상품 안내 상황극 화면: STT 음성 인식으로 답변 후 서버에 학습 세션 결과 저장
...
class _ProductScreenState extends State<ProductScreen> {
      ...
      body: SafeArea(
        child: AnimatedSwitcher(
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              children: [
                _header('상품 안내하기 👜', '손님에게 상품 정보를 정확하고 친절하게 안내해보세요.'),
                _situationBox('# 상품 안내'),
                const SizedBox(height: 6),
                _character('assets/images/shopping_girl.jpg'),
                const SizedBox(height: 16),
                Text('" ${current['guide']!} "', style: const TextStyle(fontSize: 17)),
                const SizedBox(height: 4),
                Text('${currentIndex + 1} / ${scenarios.length} 단계', style: const TextStyle(fontSize: 13, color: Colors.grey)),
  ...
  Future<void> increaseRoleplayCount() async {
    ...
    final response = await http.patch(
      Uri.parse('http://3.37.239.105:8080/api/users/statistics/roleplay'),
      headers: {
